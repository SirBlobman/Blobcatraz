...
public class CommandBan implements CommandExecutor
{
	...
	public boolean onCommand(CommandSender cs, Command c, String label, String[] args)
	{
		if(label.equalsIgnoreCase("tempban"))
		{
			...
			try
			{
				...
				{
						ConfigDatabase.tempban(toBeBanned, time * 1000L, Util.getFinalArg(args, 2)); 
						return true;
				}
				if(timeUnit.equalsIgnoreCase("m")) //Minutes
				{
					ConfigDatabase.tempban(toBeBanned, time * 60 * 1000L, Util.getFinalArg(args, 2)); 
					return true;
				}
				if(timeUnit.equalsIgnoreCase("h")) //Hours
				{
					ConfigDatabase.tempban(toBeBanned, time * 60 * 60 * 1000L, Util.getFinalArg(args, 2));
					return true;
				}
				if(timeUnit.equalsIgnoreCase("d")) //Days
				{
					ConfigDatabase.tempban(toBeBanned, time * 24 * 60 * 60 * 1000L, Util.getFinalArg(args, 2)); 
					return true;
				}
				if(timeUnit.equalsIgnoreCase("w")) //Weeks
				{
					ConfigDatabase.tempban(toBeBanned, time * 7 * 24 * 60 * 60 * 1000L, Util.getFinalArg(args, 2)); 
					return true;
				}
				if(timeUnit.equalsIgnoreCase("y")) //Years
				{
					ConfigDatabase.tempban(toBeBanned, time * 12 * 4 * 7 * 24 * 60 * 60 * 1000L, Util.getFinalArg(args, 2));
					return true;
				}
				if(timeUnit.equalsIgnoreCase("c")) //Centuries
					
				{
					ConfigDatabase.tempban(toBeBanned, time * 100 * 12 * 4 * 7 * 24 * 60 * 60 * 1000L, Util.getFinalArg(args, 2));
		...
		if(label.equalsIgnoreCase("ban") || label.equalsIgnoreCase("banish") || label.equalsIgnoreCase("exile"))
		{
			...
			if(cs.hasPermission("blobcatraz.ban"))
			{
				...
				ConfigDatabase.ban(banned, reason);
				Util.broadcast("§6" + cs.getName() + " §rbanned §2" + banned.getName() + " §rfor: §e" + Util.color(reason));
				return true;
			}
			else
			{
				cs.sendMessage(Util.noPermission + "blobcatraz.ban");
				return true;
			}
		}
		...
		if(label.equalsIgnoreCase("unban") || label.equalsIgnoreCase("pardon"))
		{
			...
			{
				cs.sendMessage(Util.noPermission + "blobcatraz.unban");
				return true;
			}
		}
	...
	{
		Util.broadcast(cs.getName() + "temporarily banned " + toBeBanned.getName() + " because: " + reason + "for: " + ConfigDatabase.getBanLengthFormatted(toBeBanned.getUniqueId()));
	}
}